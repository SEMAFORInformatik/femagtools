'''
  jcf2msh Convert JMag JPlot (or JCF) Files to Gmsh/msh

  Usage:

    jcf2msh <infile> <outfile>

  infile is either a JCF or JPlot archive file
  which includes mesh.dat or plotmesh.dat

  Author: Jonas Knöpfel, Ronald Tanner
...
def create_msh(jcfile):
    with ZipFile(jcfile, 'r') as jcf:
        nlist = jcf.namelist()
        for n in ("mesh.dat", "plotmesh.dat"):
            if n in nlist:
                with jcf.open(n, 'r') as f:
