...
def _draw_horizontal_shaft(ax,
    ...
    axis_appendix = axis_length - motor_length
    xx = (-axis_appendix,
          axis_length,
          axis_length,
          -axis_appendix)
    yy = (-axis_radius, -axis_radius, axis_radius, axis_radius)
    ax.fill(xx, yy,
            facecolor='lightgrey',
            linewidth=0,
            edgecolor='grey')
...
def _draw_horizontal_shaft_and_box(ax, param):
    stator = param.get('stator', None)
    rotor =  param.get('rotor', None)
    if not rotor or not stator:
        return
    motor_length = param.get('length', param['dy1'])

    shaft_max = min(rotor['min_radius'], stator['min_radius'])
    motor_height = max(rotor['max_radius'], stator['max_radius'])
    axis_radius = param['axis_radius']
    linewidth = max(1, axis_radius * 0.15)
    x1_r = motor_length * 1.15
    x2_r = x1_r
    x1_l = - motor_length * 0.15
    x2_l = x1_l
    y1 = axis_radius + linewidth * 4 * 1.2
    y2 = motor_height
    if param.get('external_rotor', False):
        x1_r = motor_length * 1.05
        y2 = motor_height
        y1 = axis_radius + linewidth * 4 * 1.4

    def draw_line(x1, x2, y1, y2):
        ax.add_line(plt.Line2D((x1, x2),
                               (y1, y2),
                               color='grey',
                               linewidth=linewidth,
                               linestyle='solid'))

    draw_line(x1_l, x2_l, y1, y2)
    draw_line(x1_l, x2_l, -y1, -y2)
    draw_line(x1_l, x1_r, y2, y2)
    draw_line(x1_l, x1_r, -y2, -y2)

    if not param.get('external_rotor', False):
        draw_line(x1_r, x2_r, y1, y2)
        draw_line(x1_r, x2_r, -y1, -y2)
        axis_length = motor_length * 1.3
    else:
        axis_length = motor_length * 1.2
        draw_line(x1_l - motor_length * 0.1, x1_l, y1, y1)
        draw_line(x1_l - motor_length * 0.1, x1_l, -y1, -y1)
    _draw_horizontal_shaft(ax,
                           motor_length,
                           shaft_max,
                           axis_radius,
                           axis_length)
...
def _draw_airgap(ax, param):
    ...
    if not rotor or not stator:
        return
    if param.get('external_rotor', False):
        ag2 = round(rotor['min_radius'], 1)
        ag1 = round(stator['max_radius'], 1)
    else:
        ag2 = round(stator['min_radius'], 1)
        ag1 = round(rotor['max_radius'], 1)
    xx = (0.0, motor_length, motor_length, 0.0)
    yy = (ag1, ag1, ag2, ag2)
    ax.fill(xx, yy,
            facecolor='white',
    ...
    ag_y = (ag1 + ag2) / 2.0
    ag_length = motor_length * 1.1
    ag_over = ag_length - motor_length
    ag_x1 = -ag_over
    ag_x2 = motor_length + ag_over
    ax.add_line(plt.Line2D((ag_x1, ag_x2),
                           (ag_y, ag_y),
                           color='red',
                           linewidth=2,
                           linestyle='dotted'))
...
def draw_horizontal(ax, params):
    def scale(pair_lst):
        ...
        for pair in pair_lst:
            new_pair = (round(pair[0], 1) * 10,
                        round(pair[1], 1) * 10)
            new_lst.append(new_pair)
        return new_lst

    params['length'] = round(params['length'], 1) * 10
    stator = params.get('stator', None)
    if stator:
        stator['min_radius'] = round(stator['min_radius'], 1) * 10
        stator['max_radius'] = round(stator['max_radius'], 1) * 10
        stator['wnds'] = scale(stator.get('wnds', None))
        params['stator'] = stator

    rotor =  params.get('rotor', None)
    if rotor:
        rotor['min_radius'] = round(rotor['min_radius'], 1) * 10
        rotor['max_radius'] = round(rotor['max_radius'], 1) * 10
        rotor['mags'] = scale(rotor.get('mags', None))
        rotor['fd_wnds'] = scale(rotor.get('fd_wnds', None))
        params['rotor'] = rotor

    shaft_max = min(rotor['min_radius'], stator['min_radius'])
    params['axis_radius'] = shaft_max / 3

    #_draw_horizontal_shaft_and_box(ax, params)
    _draw_horizontal_rotor(ax, params)
    _draw_horizontal_stator(ax, params)
    _draw_airgap(ax, params)
    _draw_horizontal_shaft_and_box(ax, params)


def draw_longitudinal_cut(params):
